...
export class altaClienteComponente implements OnInit {
	...
	public dniCopia: File;
	public reciboCopia: File;
	...
	seleccionImagenDni(archivo: File) {
		if ( !archivo ) {
			this.dniCopia = null;
			return;
		  }
		  ...
		  this.dniCopia = archivo;

	}

	
	seleccionImagenRecibo(archivo: File) {
		if ( !archivo ) {
			this.reciboCopia = null;
			return;
		  }
	...
	subirImagen(archivo: File, tipo: string) {
		...
		this._uploadService.subirArchivo( archivo, tipo, this.parmUrl )
			  .then( resp => {
